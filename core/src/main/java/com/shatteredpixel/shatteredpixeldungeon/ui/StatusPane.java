...
public class StatusPane extends Component {
	...
	public StatusPane(boolean large) {
		...
		if (large)
			...
			bg = new NinePatch(asset, 0, 0, 82, 38, 32, 0, 5, 0);
		add(bg);
		...
		// LAST CARAVAN has no dungeon-exit compass.
		compass.visible = false;

		if (large)
			shieldHP = new Image(asset, 0, 112, 128, 9);
		else
			shieldHP = new Image(asset, 0, 44, 50, 4);
		add(shieldHP);

		if (large)
			hp = new Image(asset, 0, 103, 128, 9);
		else
			hp = new Image(asset, 0, 40, 50, 4);
		add(hp);
